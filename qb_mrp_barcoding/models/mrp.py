...
class MRPProduction(models.Model):
    ...
    def get_barcode_view_state(self):
        ...
        for mrp in mrps:
            mrp['move_line_ids'] = self.env['stock.move.line'].browse(mrp.pop('move_line_ids')).read([
                'product_id',
                'location_id',
                'location_dest_id',
                'qty_done',
            ...
            for move_line_id in mrp['move_line_ids']:
            ...
            mrp['location_id'] = self.env['stock.location'].browse(mrp.pop('location_id')[0]).read([
                'id',
                'display_name',
                'parent_path'
            ])[0]
            mrp['location_dest_id'] = self.env['stock.location'].browse(mrp.pop('location_dest_id')[0]).read([
                'id',
                'display_name',
                'parent_path'
            ])[0]
            mrp['group_stock_multi_locations'] = self.env.user.has_group('stock.group_stock_multi_locations')
            mrp['group_tracking_owner'] = self.env.user.has_group('stock.group_tracking_owner')
            mrp['group_tracking_lot'] = self.env.user.has_group('stock.group_tracking_lot')
            mrp['group_production_lot'] = self.env.user.has_group('stock.group_production_lot')
            mrp['group_uom'] = self.env.user.has_group('uom.group_uom')
            mrp['use_create_lots'] = self.env['stock.mrp.type'].browse(mrp['mrp_type_id'][0]).use_create_lots
            mrp['use_existing_lots'] = self.env['stock.mrp.type'].browse(mrp['mrp_type_id'][0]).use_existing_lots
            mrp['show_entire_packs'] = self.env['stock.mrp.type'].browse(mrp['mrp_type_id'][0]).show_entire_packs
            mrp['actionReportDeliverySlipId'] = self.env.ref('stock.action_report_delivery').id
            mrp['actionReportBarcodesZplId'] = self.env.ref('stock.action_label_transfer_template_zpl').id
            mrp['actionReportBarcodesPdfId'] = self.env.ref('stock.action_label_transfer_template_pdf').id
            if self.env.company.nomenclature_id:
                mrp['nomenclature_id'] = [self.env.company.nomenclature_id.id]
        return mrps

    def _get_mrp_fields_to_read(self):
    ...
    def open_workorders_client_action(self):
        """ method to open the form view of the current record
        from a button on the kanban view
        """
        self.ensure_one()
        use_form_handler = self.env['ir.config_parameter'].sudo().get_param('qb_mrp_barcoding.use_form_handler')
        if use_form_handler:
            view_id = self.env.ref('mrp.action_mrp_workorder_production_specific').id
            return {
                'name': _('Open WO Tree'),
                'res_model': 'mrp.workorders',
                'view_mode': 'tree',
                'view_id': view_id,
                'type': 'ir.actions.act_window',
                'res_id': self.id,
            ...
            action = self.env.ref('qb_mrp_barcoding.mrp_barcode_production_client_action').read()[0]
            params = {
    ...
    def open_next_workorder_client_action(self):
        """ method to open the form view of the current record
        from a button on the kanban view
        """
        self.ensure_one()
        ...
        if use_form_handler:
            ...
            action = self.env.ref('qb_mrp_barcoding.mrp_barcode_workorder_client_action').read()[0]
            params = {
                'model': 'mrp.workorder',
                'production_id': self.id,
                'nomenclature_id': [self.env.company.nomenclature_id.id],
