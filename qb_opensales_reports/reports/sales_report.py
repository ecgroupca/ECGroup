...
class CompSalesXlsx(models.AbstractModel):

    _name = 'report.qb_opensales_reports.report_compsales_xlsx'
    _description = 'Comp Sales Report Xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, report):
        ...
        date_domain += [('date_order','>=',date_from.strftime("%Y-%m-%d 00:00:00"))]
        date_domain += [('date_order','<=',date_to.strftime("%Y-%m-%d 23:59:00"))]
        ...
        for sroom in sales.keys():
            ...
            sheet.write(i+j+5, 10, 'Comp Status', bold)
            sheet.write(i+j+5, 10, 'Comp Date', bold)
            ...
            for sale in sales[sroom]: 
                ...
                for sale_line in sale.order_line:
                    ...
                    if sale_line.product_uom_qty != sale_line.qty_delivered and sale_line.product_uom_qty > 0 and product and sale_line.product_id.type != 'service':
                        ...
                        sheet.write(j+i+5, 10, sale.comp_status)
                        sheet.write(j+i+5, 11, sale.comp_status_date)
                        i+=1  
                i+=1                   
...
class OpenSalesXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, report):
        ...
        for sroom in sales.keys():
            ...
            sheet.write(i+j+5, 9, 'Status', bold)
            sheet.write(i+j+5, 10, 'Comp Status', bold)
            sheet.write(i+j+5, 11, 'Comp Date', bold)
