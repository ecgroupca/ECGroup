...
class ProductSalesXlsx(models.AbstractModel):

    _name = 'report.qb_product_sales.report_product_sales_xlsx'
    _description = 'Product Sales Report Xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, report):
        ...
        """code | item name | vendor | min order qty (reordering rules) | min supplier qty (on supplier product info) | QTY_OH | max order qty | QTY_OH
        """
        query = """SELECT P.DEFAULT_CODE, P.DESCRIPTION,
            RP.NAME,ROR.MIN_ORDER_QTY,PS.MIN_ORDER_QTY,P.QTY_OH,
            ROR.MAX_ORDER_QTY
            FROM PRODUCT P
            LEFT JOIN PRODUCT_SUPPLIER_INFO PS ON PS.PRODUCT_ID = P.PRODUCT_ID
            LEFT JOIN PRODUCT_REORDER_RULES ROR ON ROR.PRODUCT_ID = P.PRODUCT_ID
            LEFT JOIN RES_PARTNER RP RP.PARTNER_ID = PS.PARTNER_ID            
            """
...
class ProductSalesReport(models.AbstractModel):
    ...
    _name = 'report.qb_product_sales_reports.report_product_sales'
