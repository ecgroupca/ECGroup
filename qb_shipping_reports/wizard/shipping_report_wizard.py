...
class ShippingReportWizard(models.TransientModel):
    ...
    def print_report(self):
        ...
        data['form'] = self.read(['company_id', 'date_from', 'date_to', 'showroom','stock_move_ids','print_selected'])[0]
        #used_context = self._build_contexts(data)
        #data['form']['used_context'] = dict(used_context, lang=get_lang(self.env).code)
        #return self.with_context(discard_logo_check=True)._print_report(data)
        return self.env.ref('qb_shipping_reports.action_report_shipping').report_action(self, data=data)