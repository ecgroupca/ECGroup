...
class WIPReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, report):
        ...
        for sroom in workcenters.keys(): 
            ...
            sheet.write(i+j+5, 6, 'Routing', bold)
            sheet.write(i+j+5, 7, 'Reserved', bold)
            sheet.write(i+j+5, 8, 'Qty', bold)
            sheet.write(i+j+5, 9, 'Status', bold)
            sheet.write(i+j+5, 10, 'Company', bold)
            sheet.write(i+j+5, 11, 'Next Workorder', bold)
            sheet.write(i+j+5, 12, 'Responsible', bold)
            ...
            for wo in workcenters[sroom]: 
                ...
                sheet.write(j+i+5, 6, wo.production_id.routing_id.name)
                sheet.write(j+i+5, 7, wo.production_id.reservation_state)
                sheet.write(j+i+5, 8, wo.production_id.product_qty)
                sheet.write(j+i+5, 9, wo.production_id.state)
                sheet.write(j+i+5, 10, wo.production_id.company_id.name)
                sheet.write(j+i+5, 11, next_wo_name)
                sheet.write(j+i+5, 12, resp_name)
                i+=1
