...
class StockValuationOnhandWizard(models.TransientModel):
    ...
    def _get_report_data(self):
        """
        Compute on-hand quantity and unit cost per (warehouse, product) as of self.date,
        grouped by warehouse so the report can show a header per parent stock location.

        Strategy:
          - On-hand qty : sum of done stock.move.line up to end of date,
                          restricted to internal locations of each warehouse separately.
        ...
        Returns a list of dicts, one per warehouse, each containing:
          {'warehouse': <name>, 'location': <root stock location display name>,
           'lines': [...], 'subtotal': <float>}
        """
        self.ensure_one()
        ...
        for wh in warehouses:
            ...
            if not locs:
                continue
            internal_location_ids = locs.ids

            # 2. Compute on-hand qty per product for THIS warehouse
            self.env.cr.execute("""
                SELECT
                    sml.product_id,
                    SUM(
            ...
            # 5. Build report lines for this warehouse
            lines = []
            ...
            subtotal = sum(l['total_value'] for l in lines)

            groups.append({
                'warehouse': wh.name,
                'location': wh.lot_stock_id.display_name,
                'lines': lines,
                'subtotal': subtotal,
            })
